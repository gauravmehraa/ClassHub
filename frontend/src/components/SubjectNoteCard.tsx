...
const SubjectNoteCard = (props: { name: string, links: [] }) => {
  ...
  return (
    <div className="bg-slate-200 glass mx-4 sm:mx-8 p-4 rounded-lg min-w-screen sm:min-w-1/3 sm:max-w-1/3">
      <div className='text-2xl font-semibold text-black mt-4 mb-6'>{props.name}</div>
      {props.links.map((link: any, index: number) => (
        <div className='mt-2 mb-4 flex flex-row flex-wrap gap-2 items-center border-t-black py-4 border' key={link._id}>
          <a
            href={link.url}
            target='blank'
